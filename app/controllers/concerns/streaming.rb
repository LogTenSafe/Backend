...
module Streaming
  ...
  PASS_THROUGH_REQUEST_HEADERS  = %w[Accept Range].freeze
  PASS_THROUGH_RESPONSE_HEADERS = %w[Content-Type Content-Length Range Accept-Range].freeze
      ...
      http.request(outbound_request) do |outbound_response|
        case outbound_response
          when Net::HTTPRedirection
            stream outbound_response['Location']
          else
